...
当主存储过程体内顺序调用了多个子流程，Aggregate 按原 PL/SQL 顺序为每个步骤生成一个方法，Processor 按序编排，**Processor 不含任何业务逻辑**，只做调用编排 + 异常捕获 + 状态流转。

```java
// Aggregate：每个业务步骤一个方法
@Component
...
**规约要点：**
1. 【强制】统一使用 `TranFailException` 作为业务异常类型，禁止抛出 `new RuntimeException()`、`Exception` 或 `Throwable`。各层方法声明 `throws TranFailException`。
2. 【强制】异常信息长度超过 1000 字符时必须截断，避免数据库字段溢出。
3. 【强制】Processor 层捕获异常后必须更新 `procStat` 和 `expInfo`。
4. 【强制】catch 块不得丢弃原始异常信息：必须包装重抛（`throw new TranFailException(msg, e)`）或经 `CommonLog.error(msg, e)` 记录完整堆栈；禁止空 catch、禁止仅打印 `e.getMessage()` 丢弃堆栈。
...
10. 【强制】IDE 的 text file encoding 设置为 UTF-8；换行符使用 Unix 格式。
...
4. 【强制】外部正在调用或者二方库依赖的接口，不允许修改方法签名。接口过时必须加 @Deprecated 注解。
5. 【强制】不能使用过时的类或方法。
...
19. 【推荐】类成员与方法访问控制从严——构造方法最小可见、静态工具类私有构造禁止 public、成员变量和方法最小可见原则。（注：`@Component` 注解的 Bean/Builder/Validator/Aggregate 等由 Spring 实例化，构造方法保留 public；本条针对静态工具类。）
...
3. 【强制】使用 Map 的方法 keySet()/values()/entrySet() 返回集合对象时，不可以对其进行添加元素操作。
4. 【强制】Collections 类返回的对象如 emptyList()/singletonList() 等都是 immutable list，不可对其进行添加或者删除元素的操作。
5. 【强制】在 subList 场景中，对原集合元素的增加或删除，均会导致子列表抛 ConcurrentModificationException 异常。
6. 【强制】使用集合转数组的方法，必须使用集合的 toArray(T[] array)，传入类型完全一致、长度为 0 的空数组。
7. 【强制】使用 Collection 接口任何实现类的 addAll() 方法时，都要对输入的集合参数进行 NPE 判断。
8. 【强制】使用工具类 Arrays.asList() 把数组转换成集合时，不能使用其修改集合相关的方法。
9. 【强制】泛型通配符 <? extends T> 来接收返回的数据，此写法的泛型集合不能使用 add 方法，而 <? super T> 不能使用 get 方法。
10. 【强制】在无泛型限制定义的集合赋值给泛型限制的集合时，使用集合元素时需进行 instanceof 判断。
11. 【强制】不要在 foreach 循环里进行元素的 remove/add 操作。remove 元素请使用 Iterator 方式，并发操作需对 Iterator 对象加锁。
12. 【强制】在 JDK7 版本及以上，Comparator 实现类要满足三个条件（对称性、传递性、一致性）。
13. 【推荐】集合泛型定义时，使用 diamond 语法或全省略。
14. 【推荐】集合初始化时，指定集合初始值大小。
15. 【推荐】使用 entrySet 遍历 Map 类集合 KV。
16. 【推荐】高度注意 Map 类集合 K/V 能不能存储 null 值的情况。
17. 【参考】合理利用好集合的有序性(sort)和稳定性(order)。
18. 【参考】利用 Set 元素唯一的特性，快速对一个集合进行去重操作。

## 八、控制语句

1. 【强制】在一个 switch 块内，每个 case 要么通过 continue/break/return 等来终止，要么注释说明程序将继续执行到哪一个 case 为止；都必须包含一个 default 语句并放在最后。
...
6. 【推荐】除常用方法外，不要在条件判断中执行其它复杂的语句，将复杂逻辑判断的结果赋值给一个布尔变量名。
...
11. 【参考】下列情形需要进行参数校验：调用频次低的方法、执行时间开销很大的方法、对外提供的开放接口、敏感权限入口。
...
> 以下为**单个存储过程**的翻译落位顺序（Builder→Validator→Aggregate→…→Access）；与工作流 stage 顺序是两个维度，勿混淆。
...
1. 【强制】禁止 `var`（Java 10+）。
2. 【强制】禁止 `List.of()`/`Map.of()`/`Set.of()`（Java 9+），用 `Collections.unmodifiableList()`/`Arrays.asList()` 替代。
3. 【强制】禁止 `Optional.ifPresentOrElse()`/`Optional.or()`/`Optional.stream()`（Java 9+）。
4. 【强制】禁止 `Stream.takeWhile()`/`dropWhile()`/`ofNullable()`（Java 9+）。
5. 【强制】禁止 `String.isBlank()`/`strip()`/`lines()`/`repeat()`（Java 11+），用 `trim()` 和自行实现。
6. 【强制】禁止 `HttpClient`（Java 11+）。
7. 【强制】禁止 `record`（Java 16+）。
8. 【强制】禁止 `sealed`/`permits`/instanceof 模式匹配（Java 16+）。
9. 【强制】禁止文本块 `"""`（Java 15+）。
10. 【强制】禁止 switch 表达式（Java 14+）。
11. 【强制】Lambda 和 Stream API 可正常使用（Java 8 已引入），但不得使用 Java 9+ 新增增强。
