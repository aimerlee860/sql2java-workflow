...
7. **变量声明位置**：非局部变量声明在主函数顶层；未识别变量已查依赖签名块/`source.sql` 确认常量声明，未硬编码。
...
- 文件名去 SQL 前缀、PascalCase、业务含义、类型后缀（按架构模型段角色 `suffix`：AccessIntf/AccessImpl/Processor/Aggregate/Builder/Validator/Mapper）。
- 包路径层级正确（DDD 有根包 `com.example.mfgerp`，按 `{packageBase}/{module}/<层>` 组织，per-proc 类按角色落对应目录，文件名用 `procClassNames.className` 派生）。
