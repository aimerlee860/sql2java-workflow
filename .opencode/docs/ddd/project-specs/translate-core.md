...
6. **SQL 与函数定义一致**：Mapper XML 中 SQL 必须与 `shard-inputs/{pkg}/{ref}/source.sql` 函数定义逐行一致，禁私自优化/简化/改写。**例外（GaussDB 方言适配，不算改写）**：`FROM DUAL` → `FROM sys_dummy`（见 §五）、表名补 schema 前缀（见 §四规则 2）、XML 比较符号转义/CDATA。
