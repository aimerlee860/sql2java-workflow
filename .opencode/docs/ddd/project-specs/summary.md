# Project Spec — summary 子阶段（DDD：翻译总结稿生成）

> 本规约由引擎注入 translate-summary 子 agent 系统提示词（DDD 架构）。为本 unit 生成**翻译总结稿**：
> 回顾实际实施，并对照 skeleton 产出的 FSD 设计稿 `fsd/{pkg}/{ref}.md` 做「设计 vs 实施偏差对照」。
> 供**人工审核**翻译质量用，**孤立产出**——不回填影响翻译。

## 一、定位

为本 unit 生成**翻译总结稿**：回顾实际实施，并对照 skeleton 产出的 **FSD 设计稿** `fsd/{pkg}/{ref}.md` 做「设计 vs 实施偏差对照」。读源码 `shard-inputs/{pkg}/{ref}/source.sql` + 翻译决策 `translations/{pkg}/{ref}.json` 的 `decisions` + 依赖签名块 + 设计稿 `fsd/{pkg}/{ref}.md`，按 6 板块填**实际**值 + 第 7 偏差对照板块。
...
- **DDD 分层落位**：记录 PL/SQL 逻辑实际落到哪层（Access/Processor/Aggregate/Builder/Validator/OutService），与设计稿计划对照。
- **存储过程调用**（CALL/跨包 PROCEDURE）单独列出 OUT/IN OUT 参数清单 + OutService 映射。
- **事务边界**（COMMIT/ROLLBACK/PRAGMA AUTONOMOUS_TRANSACTION）标注为事务构造，具体 Java 事务映射见注入的 Java 代码规约（DDD Aggregate `@Transactional(rollbackFor=Exception.class)`）。
- **异常处理策略**：若 translate-core 产出含 try-catch，在板块 6 记录异常捕获策略（Processor catch 内 CommonLog.error + 设 procStat/expInfo 不外抛；Aggregate/Validator throws TranFailException；>1000 字符截断）。

## 六、质量要求

- **总结稿自包含**：每个板块写实质内容，**禁止"详见 xxx"占位符**。全程中文输出，专有名词与关键字保持英文。
- refName 用 inventory 算好的（重载带 `__序号`）。板块编号/标题与设计稿 `fsd/{pkg}/{ref}.md` 完全对齐，便于人工逐板块 diff。
...
- 不改翻译产物（只读 Java/decisions/设计稿，不 edit）。禁止调用 workflow 工具的任何 action。
