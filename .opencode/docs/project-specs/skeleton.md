...
- Mapper/Service 方法签名**按参数数量派生，禁 `Map<String,Object>` 传参**（规约 §3.2）：
  - IN 参数 >1 → 入参用 `{className}Request`（建文件，见下）；IN =1 → 直传该参数；IN =0 → 无入参。
  - OUT 参数 >1 → 返回 `{className}Response`（建文件，见下）；OUT =1 → 直返该类型；OUT =0 → `void`。
  - Mapper 接口方法签名对应本过程将用到的 SQL 语句（SQL 体由 translate-core 填），与 Service 接口签名同源。
- 桩体：`return null;` / `return 0;` / `return false;` 等默认值 + `// TODO: [translate] 标记人 标记时间 中文说明原因`，保证可编译。
- **`{className}Request`/`{className}Response`（条件参数 DTO）**：仅当 IN 参数 >1 / OUT 参数 >1 时分别生成（落位按架构模型段 `request`/`response` 角色，默认 `service/dto/request/{className}Request.java`、`service/dto/response/{className}Response.java`）。用 `@Data` 注解；字段名/类型必须与 `source.sql` 的 IN/OUT 参数一致，不一致标 `// TODO: [translate]`，禁编造字段。1 参数直传不建此类。
...
- [ ] 对照 scaffold procClassNames + packageMappings，本 unit 必建角色（service/service-impl/mapper）per-proc 文件均已创建，文件名用 className 派生、路径按角色顶层包；条件角色 request/response 仅 >1 入参/出参时建（见 §六）
- [ ] 无遗漏、无增加（除角色集模板 + 条件 request/response 外不擅自加文件）
- [ ] 未覆盖/删除/修改任何已存在文件
- [ ] 桩体可被 javac parse 通过
- [ ] mapper/service 方法签名按参数数派生（>1 入参用 `{className}Request`、>1 出参用 `{className}Response`、1 参数直传），禁 `Map<String,Object>` 传参
