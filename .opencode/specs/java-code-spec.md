...
@Getter
@Setter            // 访问器交 Lombok 生成，省手写 getter/setter 篇幅
public class FOrderStateDTO {
    // 包级可变变量：实例字段（Lombok @Getter/@Setter 生成访问器，session 作用域 bean，每会话独立）
...
**规约要点：**
1. 【强制】包级可变变量（inventory `variables`）→ session 作用域 bean 的实例字段 + Lombok `@Getter`/`@Setter`（`@Component @Scope("session")`），不得用 `static` 可变字段（线程不安全）。**禁止手写 getter/setter**；不用 `@Data`（可变 session 状态不需 equals/hashCode/toString）。
...
4. 【强制】变量名派生 Java 字段名时先剥 PL/SQL 全局变量前缀 `g`/`g_`（如 `g_log_type` → `logType`、`gCurrBizDate` → `currBizDate`），不得把 `g` 前缀带进字段名生成 `getGLogType()`/`setGCurrBizDate()` 这种 `getXg...` 形态。`g` 后跟小写字母属单词本体（如 `gold_price`）不剥。
...
> `{Proc}` = 过程/函数名转 PascalCase：**先剥 PL/SQL 无意义前缀 `F_`/`P_`/`R_`**（F=function/P=procedure/R=routine 约定前缀，仅单字母+下划线，如 `F_GET_BO_TRADE_ID` → `GET_BO_TRADE_ID`、`R_PRE_CHECK_DATE` → `PRE_CHECK_DATE`；`FIND_ITEM`/`REMOVE_ITEM` 等首段非单字母前缀不剥），再去 `__序号` 重载后缀，下划线分段首字母大写（如 `CREATE_ORDER` → `CreateOrder`、`f_get_item__1` → `GetItem`）；**跨包同名过程碰撞时由 scaffold 全局去重，首现保持 `{ProcPascal}`，后续加数字后缀 `{ProcPascal}2`/`{ProcPascal}3`**（去重后基名记为 `{ResolvedBase}`，落 `scaffold.json.generated.procClassNames`）；`{Pkg}` = PL/SQL 包名转 PascalCase；`{Table}` = 表名转 PascalCase。Java 顶层包段全小写（`service.impl`）。
...
8. 【强制】过程/函数名派生 Java 类基名时，先剥 PL/SQL 无意义前缀 `F_`/`P_`/`R_`（见上 `{Proc}` 定义），不得把 `F_`/`R_` 等约定前缀带进 Java 类名（如 `F_GET_BO_TRADE_ID` → `GetBoTradeId`，非 `FGetBoTradeId`）。引擎 fallback `procNameToPascal` 同源剥离，保证 scaffold 漏盖时一致。
