...
执行 **translate / test-gen** 子阶段：为本 unit 的 `{Proc}ServiceImpl.java` 生成单元测试（Mockito）。**仅 ServiceImpl，不生成 Mapper 集成测试**。方法论见 agent 指南（translate-test.md）。
...
- per-proc 测试 Java 文件：`src/test/java/service/impl/{className}ServiceImplTest.java`（无根包，按角色顶层包；`className` 见上方「本 unit 文件清单」已注入，跨包同名已去重，勿查 scaffold.json；各 unit 独占测试文件无冲突）。
