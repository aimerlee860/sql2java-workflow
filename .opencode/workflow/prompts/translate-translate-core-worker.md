...
- **禁 Map 传参**：mapper/service 方法签名由 skeleton 已按参数数量定好（>1 入参用 `{className}Request`、>1 出参用 `{className}Response`、1 参数直传，规约 §3.2）。填方法体时经 Request getter 取参、Response setter 回参，**禁止 `new HashMap`/`Map<String,Object>` 凑参**；mapper XML 的 `#{}` 占位符直接对 Request 字段（`#{fmBranchId}` ← `request.getFmBranchId()`）。
