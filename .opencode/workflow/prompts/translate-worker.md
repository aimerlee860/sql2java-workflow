...
- ⛔ **源码只读 `shard-inputs/{pkg}/{ref}/source.sql`**（引擎已预切）。禁止 read 整包 body/header、`packages/{pkg}.json`、`analysis-packages/{pkg}.json`。analyze 砍后不再有 analysis-slice，直接读 source.sql 理解结构。
