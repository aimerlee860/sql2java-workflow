...
1.6 **项目规范查找**
   ...
   - **有规约文件** → 传递给 workflow start（用户 spec **整体替换**内置默认规约，不再合并）
   - **无规约文件** → 使用内置默认规约（`.opencode/specs/java-code-spec.md`）和 Maven 结构
   ...
   **`@include` 引用机制**：主规约（默认或用户 `--spec`）支持两条指令——
   ...
   路径相对主 spec 文件目录 resolve，递归 include，缺失文件 warn 保留原行。默认主规约已用 7 条 `-> agent` 路由分发 `translation-specs/`，用户 `--spec` 可重写路由与架构模型以切换架构（如 DDD，完整 DDD 规约见 `.opencode/specs/ddd/java-code-spec.md`，`--spec` 指向它即按 DDD 跑）。
