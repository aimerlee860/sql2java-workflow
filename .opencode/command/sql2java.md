...
9. **⛔ 禁止凭记忆回顾进度** — dispatch/advance 输出里的 `📊 进度:` 行（如 `analyze✓ | translate 7/13`）是引擎权威状态，**以此为准**。长流程（多阶段 × 多分片）上下文很长，**禁止**自己"回顾当前进度/审视进展"——那会凭模糊记忆 confabulate 出错误数字（如误报已完成阶段为 7/13）。不确定时调 `workflow({ action: "status", runId })` 查权威状态，不要猜。
