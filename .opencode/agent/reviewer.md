...
<!-- Java 代码规约由引擎自动注入系统提示（默认 specs/java-code-spec.md；--spec 指定时整体替换为用户规约——以实际注入内容为准，勿自行 read 规约文件），无需在此重复 -->
