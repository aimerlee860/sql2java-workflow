...
- **分片模式**（`targetUnits` 存在且 `shardIndex` 存在）：
  ...
  - 只读本分片 unit 的源码片段（按 lineRange）+ FSD + per-package 文件；不要一次性读全部源码
  - 仍按 `procedureOrder` 顺序处理这些 unit（跳过不在 targetUnits 中的）
  - 依赖的已翻译 unit 所属包的聚合 `translation.json` 路径已在 upstreamArtifacts 中给出，直接 `read`
    （仅限跨包/同包跨单元调用对接，不要顺带处理这些包）
...
2. **读取 Oracle 源码**：按 `inventory-packages/{PKG}.json` 的 `procedures[].lineRange` **只抽取本单元
   各子程序的源码片段**（非整包），降低上下文。lineRange 为 `[起行, 止行]`，用 bash `sed -n` 截取。
