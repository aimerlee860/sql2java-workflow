...
**文件：`plugin/workflow-engine.ts:420-457`**
...
1. **扫描目录结构**：`find` 命令列出所有 `.sql`/`.pks`/`.pkb` 文件
2. **逐文件解析**：识别 Package spec/body、DDL、Trigger、View、Sequence、独立子程序
3. **提取包结构**：每个 Package 的过程/函数签名、参数（name, oracleType, direction）、返回类型、行号范围
4. **提取表结构**：列名、Oracle 类型、是否可空、是否主键
5. **提取其他对象**：触发器、视图、序列
6. **写入 `inventory.json`** 到 `.workflow-artifacts/run-20260602-143000/inventory.json`
...
### advance 内部流程（`engine-core.ts:147-306`）

1. **验证** run.status === "running"
2. **Zod 校验**（D5）：`validateArtifactOnDisk` 从磁盘读取 `inventory.json`，用 `InventorySchema` 校验结构
   - 校验失败 → 返回 rejection，LLM 需修复 artifact
   - 校验通过 → 继续
3. **跨 Schema 校验**（D9）：首次没有 analysis，跳过
...
System prompt 重新构建，注入 analyze phase section + Runtime Context（含 `upstreamArtifacts: inventory.json`）。
...
- Zod 校验 `analysis.json` → `AnalysisSchema`
- 跨 Schema 校验：inventory 包名 ↔ analysis 包名双向一致性、translationOrder 覆盖校验
...
**注意**：plan 阶段不是直接执行的。引擎返回 `waitingForConfirmation: true` 后：

**命令层（sql2java.md 分支 4 步骤 5）** 向用户输出提示：

> Plan 阶段完成，等待确认。请审阅 plan.json 后调用：
> `workflow({ action: "confirm", runId: "run-20260602-143000" })`

**但这里有个关键时序**：plan 阶段还没有执行！返回 `waitingForConfirmation` 只是设置了 `status: "pending"`。实际上 LLM 此时会**以 java-architect 身份执行 plan 阶段**的工作：

1. 读取 `inventory.json` + `analysis.json`
...
- 跨 Schema 校验：plan 的 packageMappings 覆盖 inventory 所有包
- 匹配规则：`plan → condition: "always" → scaffold`
- **plan 的 `requiresConfirmation: true`** → 引擎设 `status: "paused"`，`waitingForConfirmation: true`

**用户确认**：用户审阅 plan.json 后，调用 `workflow({ action: "confirm", runId })`

- 引擎将 status 从 `"paused"` 改为 `"running"`
- currentEntry 从 `"pending"` 改为 `"in_progress"`
...
**advance 时**（`engine-core.ts:244-258`）：在创建 fix entry 前，检查 **D2 双层 exhausted**：
...
### fix advance 特殊处理（`engine-core.ts:577-747`）
...
① inventory (sql-analyst, temp=0.1)
  │  扫描源码 → 写入 inventory.json
  │  advance → Zod 校验 → inventory→analyze (always)
  │
  ▼
② analyze (sql-analyst, temp=0.1)
  │  依赖图 + 拓扑排序 + 子程序解析 + FSD 生成 → 写入 analysis.json + fsd/
  │  advance → Zod 校验 + 跨 Schema 校验 → analyze→plan (always)
  │
  ▼
③ plan (java-architect, temp=0.2)
  │  架构规划 → 写入 plan.json
  │  advance → requiresConfirmation=true → status="paused"
  │  ⏸️ 等待用户确认
  │  用户调用 confirm → status="running"
  │
  ▼
...
| D3 | fix 增量重做 | fix 后只重新处理修复过的包（targetPackages） |
| D4 | confirm 时序 | plan advance 后 status 暂停，用户显式确认后才继续 |
| D5 | Zod 校验 | advance 时从磁盘读 artifact 做 Zod 结构校验，校验失败阻止前进 |
| D6 | 持久化 | 每次状态变更都写 run.json + _events.log，支持崩溃恢复 |
| D7 | fix 动态路由 | fix 不写死 transitions，由 handleFixAdvance 根据 branchedFrom 动态回环 |
| D8 | result 推导 | review/verify 阶段从 summary 的 allPassed 自动推导 result |
| D9 | 跨 Schema 校验 | inventory ↔ analysis ↔ plan 的包名一致性、覆盖完整性 |
