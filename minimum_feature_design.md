...
- 15 类审查清单：逻辑等价、SQL 完整性、空值处理、类型映射、异常映射、事务边界、游标映射、参数方向、命名一致性、TODO 残留统计、命名规约（UpperCamelCase/lowerCamelCase/常量全大写/ServiceImpl 后缀）、代码格式（4 空格缩进/120 字符行宽）、OOP 规约（@Override/包装类型/BigDecimal 精度）、注释规约（中文注释/Javadoc 格式）、集合与异常（初始化大小/entrySet/try-with-resources/禁止空 catch）
...
| reviewer | review | 15 类审查清单（含命名规约/代码格式/OOP/注释语言/集合异常），逐包持久化 | translations/{pkg}/review.json + review-summary.json |
| reviewer | verify | mvn compile + MyBatis 校验 + 完整单元测试生成，逐包持久化 | translations/{pkg}/verify.json + verify-summary.json |
...
- 定义所有 Zod Schema（InventoryIndexSchema + InventoryPackageSchema + InventorySchema + AnalysisMetaSchema + AnalysisPackageSchema + PlanSchema + ScaffoldSchema + TranslationSchema + ReviewSchema（15 类 category）+ ReviewSummarySchema + VerifySchema + VerifySummarySchema + FixArtifactSchema + refine 约束）
...
- reviewer.md（review 15 类审查清单 + verify 编译校验 + 完整单元测试生成 + 增量模式，规约注入占位）
...
19. **15 类审查清单验证**：review 阶段的 category 枚举包含新增的 5 类（naming-convention / code-format / oop-convention / comment-convention / collection-exception）
