...
- 统一 Java 代码规约定义在 `specs/java-code-spec.md`，工作流引擎构建 system prompt 时自动注入
- **注入对象**：java-architect（plan + scaffold）、translator（translate + fix）、reviewer（review + verify）三个 agent
- **注入方式**：plugin 构建系统提示时在 agent .md 的 `<!-- Java 代码规约由引擎从 specs/java-code-spec.md 自动注入 -->` 注释位置插入规约内容
...
- **维护策略**：修改 `specs/java-code-spec.md` 即可全局生效，无需同步修改多个 agent 文件
...
5. 读取 specs/java-code-spec.md（D19：仅 java-architect / translator / reviewer）
6. 拼接：
   [通用规则]
...
8. **Java 代码规约**（java-architect / translator / reviewer）：引擎自动从 `specs/java-code-spec.md` 注入统一规约（D19）
...
- **代码规约**：`specs/java-code-spec.md`（自动注入 agent system prompt，D19）
- **LLM**：Claude API
...
### Step 4: agent .md 文件 + specs/java-code-spec.md ✅
...
- specs/java-code-spec.md（统一 Java 代码规约，12 板块，自动注入 3 个 agent，D19）
...
18. **Java 代码规约注入验证**（D19）：java-architect / translator / reviewer 三个 agent 的 system prompt 中包含 specs/java-code-spec.md 的规约内容
